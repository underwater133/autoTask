...
- 🔁 失败重试：最多 3 次尝试，间隔统一 30 秒，重试耗尽后**震动提醒**
...
### 失败重试与通知
- 启动失败自动重试 3 次（间隔统一 30 秒），每次尝试记录日志
...
- **vivo"后台弹出界面"权限**（重要）：默认禁止第三方 App 在后台拉起 Activity（悬浮窗权限无法豁免），表现为"启动成功但目标应用未打开"。
  需在 **i管家 → 应用管理 → 权限管理 → 后台弹出界面** 中允许 autoTask（权限引导页已提供一键跳转入口）
